...
static t_list *merge_sorted_lists(t_list *smaller, t_list *equal, t_list *greater)
{
	t_list	*result;
	t_list	*tmp;
...
static void partition_list(t_list *list, t_list *pivot, int (*f)(void *, void *),
...
t_list	*ft_qsort(t_list *list, int (*f)(void *, void *))
{
	...
	if (!list || !list->next)
		return (list);
	partition_list(list, list, f, &smaller, &equal, &greater);
	smaller = ft_qsort(smaller, f);
	greater = ft_qsort(greater, f);
	return (merge_sorted_lists(smaller, equal, greater));
}
